...
//            $0.center.equalToSuperview()
        ...
        blueBox.snp.makeConstraints { make in
            ...
            //make.width.height.equalTo(redBox.snp.width).dividedBy(2) // 축소배율
            make.width.height.equalTo(redBox.snp.width).multipliedBy(2) // 확대배율
            
//            make.width.equalTo(redBox.snp.width)
//            make.height.equalTo(redBox.snp.height)
            //make.width.height.equalTo(100)
            
            
            // view의 snp의 ~~로 접근해야함 (view.snp.(attribute)
            make.top.equalTo(redBox.snp.bottom).offset(100)
        ...
        greenBox.snp.makeConstraints { make in
            make.width.height.equalTo(100)
            make.centerX.equalToSuperview()
            // ( "target".snp."attribute" ) 로 접근해야함
            // offset은 constraints와 같음
...
//        ])
    ...
    @objc fileprivate func moveGreenBoxDown() {
        offset += 40
        print("ViewController - moveGreenBox() called (offset = \(offset))")
        
        self.greenBoxTopConstraint?.update(offset: offset)
        UIViewPropertyAnimator(duration: 0.5, curve: .easeOut, animations: self.view.layoutIfNeeded).startAnimation()
